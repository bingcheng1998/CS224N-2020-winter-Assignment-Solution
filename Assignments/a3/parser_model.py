...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.ones(self.n_features * self.embed_size, self.hidden_size))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        self.embed_to_hidden_bias = nn.Parameter(torch.ones(self.hidden_size))
        nn.init.uniform_(self.embed_to_hidden_bias)

        self.hidden_to_logits_weight = nn.Parameter(torch.ones(self.hidden_size, self.n_classes))
    ...
    def forward(self, w):
        ...
        h = self.relu(x@self.embed_to_hidden_weight + self.embed_to_hidden_bias)
        h = self.dropout(h)
