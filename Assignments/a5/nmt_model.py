...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(input_size=embed_size, bias=True,
                               hidden_size=self.hidden_size, dropout=self.dropout_rate, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # print(source_padded.size())
        X = self.model_embeddings_source(source_padded)

        input = pack_padded_sequence(input=X, lengths=source_lengths)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            for prev_hyp_id, hyp_word_id, cand_new_hyp_score in zip(prev_hyp_ids, hyp_word_ids, top_cand_hyp_scores):
                ...
                # print(f'type of prev_hyp_id is {type(prev_hyp_id)}, prev_hyp_id = {prev_hyp_id}')
                # prev_hyp_id
